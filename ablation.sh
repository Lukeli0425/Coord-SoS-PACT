...
#   python reconstruction.py --task numerical --sample_id 5 --n_delays $n_delays_df --hls $hls_df --hfs $hfs_df --n_epochs 30 --lr $lr --batch_size $bs_df --gpu $gpu
# done
# for lr in {1e-2,5e-3,2e-3,1e-3,5e-4,2e-4,1e-4}; do
...
# for sample_id in {5,6,7,8,9}; do
#   for n_delays in {64,32,16,8,4,2,1}; do
#     python reconstruction.py --task numerical --sample_id $sample_id --n_delays $n_delays --hls $hls_df --hfs $hfs_df --n_epochs $n_epochs_df --lr $lr_df --batch_size $bs_df --gpu $gpu
#   done
# done

# for n_delays in {64,32,16,8,4,2,1}; do
#   python reconstruction.py --task in_vivo --n_delays $n_delays --hls $hls_df --hfs $hfs_df --n_epochs $n_epochs_df --lr $lr_df --batch_size $bs_df --gpu $gpu
# done

# for n_delays in {64,32,16,8,4,2,1}; do
#   python reconstruction.py --task phantom --n_delays $n_delays --hls $hls_df --hfs $hfs_df --n_epochs $n_epochs_df --lr $lr_df --batch_size $bs_df --gpu $gpu
# done
...
#   python reconstruction.py --task numerical --sample_id 5 --n_delays $n_delays_df --method PG --lam_tv $lam_tv_df --n_epochs 30 --lr $lr --batch_size $bs_df --gpu $gpu
# done
