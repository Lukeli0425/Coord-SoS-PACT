...
#     python reconstruction.py --task numerical --hidden_fts $hidden_fts --hidden_lyrs $hidden_lyrs --lam_tv 5e-5 --gpu $gpu
#   done
# done

# for hidden_fts in {32,64,128,256}; do
#   for hidden_lyrs in {1,2}; do
#     python reconstruction.py --task in_vivo --hidden_fts $hidden_fts --hidden_lyrs $hidden_lyrs --lam_tv 1e-5 --gpu $gpu
#   done
# done

for hidden_fts in {32,64,128,256}; do
  for hidden_lyrs in {1,2}; do
    python reconstruction.py --task phantom --hidden_fts $hidden_fts --hidden_lyrs $hidden_lyrs --lam_tv 5e-5 --gpu $gpu
  done
done
...
for lr in {5e-4,2e-4,1e-4,5e-5,2e-5}; do
  python reconstruction.py --task phantom --lam_tv 5e-5 --lr $lr --n_iters 50 --gpu $gpu
done
...
for lam_tv in {0,1e-5,2e-5,5e-5,1e-4}; do
  python reconstruction.py --task phantom --lam_tv $lam_tv --n_iters 30 --gpu $gpu
done
